...
int
shmht_iterate (struct shmht *h, int (*consumerf) (void *))
{
	void *retValue = NULL;
	struct internal_hashtable *iht = h->internal_ht;
	if (read_lock (iht->semaphore) < 0)
	...
	int i, iterated_count; 
	//Iterate over all the buckets:
	for (i = 0, iterated_count = 0; i < iht->tablelength && iterated_count < iht->entrycount; i++, iterated_count++) {
		struct bucket *bucket_ptr = h->bucketmarket +
			(i * (sizeof (struct bucket) + iht->registry_max_size));
                if (bucket_ptr->used) {
			retValue = h->bucketmarket +
				(i * (sizeof (struct bucket) + iht->registry_max_size))
				+ sizeof (struct bucket);
			if (consumerf(retValue) < 0) break;
                }
	}

	read_unlock (iht->semaphore);
	return iterated_count;
